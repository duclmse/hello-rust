...
use crate::utils::{clone_handle, win_error_to_io};
use std::ffi::c_void;
...
impl Read for PipeReader {
  fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
    ...
    let result = unsafe { ReadFile(self.handle, buf.as_mut_ptr() as _, buf_size, &mut n, null_mut()).ok() };
    match result {
      Ok(()) => Ok(n as usize),
      ...
      Err(err) if err.code() == HRESULT::from_win32(232) => {
        Err(io::Error::new(io::ErrorKind::WouldBlock, err.message().to_string()))
      },
...
impl Write for PipeWriter {
  ...
  fn flush(&mut self) -> io::Result<()> {
    unsafe {
      FlushFileBuffers(self.handle).ok().map_err(win_error_to_io)?;
    }
    Ok(())
  }
}
