  ...
  fn path(&self) -> Option<String> {
    let path = match &self.local_base_path {
      Some(local_base_path) => match &self.common_path_suffix {
        ...
        Some(common_path_suffix) => Some(format!("{}\\{}", local_base_path, common_path_suffix).replace("\\\\", "\\")),
...
impl LinkInfoFlags {
  pub fn new(VolumeIDAndLocalBasePath: bool, CommonNetworkRelativeLinkAndPathSuffix: bool) -> Result<LinkInfoFlags> {
    Ok(LinkInfoFlags {
